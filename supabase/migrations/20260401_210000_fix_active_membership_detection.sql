-- ============================================================================
-- Fix: detect active membership reliably before renewing
-- Fecha: 2026-04-01
-- Proposito:
-- 1. Evitar que una renovacion intente insertar una segunda membresia activa
-- 2. Reemplazar el chequeo compuesto IS NOT NULL por una bandera explicita
-- 3. Mantener la acumulacion de clases sobre la membresia activa
-- ============================================================================
...
BEGIN
  ...
  v_has_active_membership := v_active_membership.id IS NOT NULL;
  ...
  IF v_payment_amount IS NOT NULL THEN
    ...
    VALUES (
      ...
      NULL,
      CASE
        WHEN v_has_active_membership THEN 'Pago registrado al renovar la membresia'
        ELSE 'Pago inicial registrado al vender la membresia'
      END,
      CASE
        WHEN v_has_active_membership THEN 'admin_renewal'
...
COMMENT ON FUNCTION public.admin_assign_membership_plan(uuid, uuid, date, numeric, numeric, text) IS
  'Vende o renueva una membresia V2 para un alumno. Detecta la membresia activa con una bandera explicita para acumular clases sin violar el indice de una sola activa.';
