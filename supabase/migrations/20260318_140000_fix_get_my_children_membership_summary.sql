-- ============================================================================
-- FIX: HUB TUTOR - RESUMEN DE MEMBRESIA EN get_my_children
-- Fecha: 2026-03-18
-- Proposito:
-- 1. Evitar "Sin membresia" falso cuando existe membresia V2
-- 2. Priorizar membresia activa vigente para hoy (America/Lima)
-- 3. Devolver estado "expired" cuando corresponde para UX clara
-- ============================================================================
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    ...
    ORDER BY
      CASE
        -- Mejor caso: membresia activa y vigente hoy
        ...
        THEN 0
        -- Activa pero no vigente (datos legados / estado desfasado)
        WHEN sm_inner.status = 'active' THEN 1
        -- Luego cualquier otra membresia para no dejar vacio el contexto
        ELSE 2
      END,
...
COMMENT ON FUNCTION public.get_my_children() IS
  'Lista alumnos accesibles (self/guardian) con resumen robusto de membresia y proxima reserva.';
