...
export default function EditarReservaPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const loadBookingPage = async () => {
      ...
      try {
        ...
        const { data: bData, error: bError } = await supabase.rpc('get_booking_detail', { p_booking_id: params.id })
        ...
        toast.push({ message: loadError?.message || 'No se pudo cargar la pagina de edición.', type: 'error' })
      } finally {
        setLoading(false)
      }
    }
  ...
  }, [activeStudentId, month, params.id])
  ...
  async function editarReserva(newSessionId: string) {
    ...
    try {
      setBooking(true)
      const { data, error } = await supabase.rpc('update_booking_session', {
      ...
      toast.push({ message: 'Reserva modificada.', type: 'success' })
      router.push(`/reserva/${params.id}`)
    } catch (bookingError: any) {
      toast.push({ message: bookingError?.message || 'No se pudo modificar la reserva.', type: 'error' })
    } finally {
      setBooking(false)
    }
  }
    ...
    <div className="p-5 space-y-5">
      {account?.role === 'guardian' && activeStudent && students.length > 1 && (
        <div className="card p-4 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm text-textsec">Editando reserva de</p>
      ...
      {originalBooking && (
        <div className="card p-4 border-l-4 border-accent">
          <p className="text-sm text-textsec mb-1">Horario actual:</p>
      ...
      <header className="flex items-center justify-between">
        <div>
          <button onClick={() => router.back()} className="text-sm text-textsec mb-1 hover:text-white transition-colors">Volver</button>
          <h1 className="text-lg font-semibold">Seleccionar nuevo turno</h1>
        </div>
            ...
            return (
              <div key={session.session_id} className="card p-4 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => editarReserva(session.session_id)}
                  disabled={disabled}
                >
                  {session.already_reserved ? 'Tu horario asignado' : booking ? 'Guardando...' : 'Cambiar a este horario'}
                </button>
              </div>
            )
          })}
