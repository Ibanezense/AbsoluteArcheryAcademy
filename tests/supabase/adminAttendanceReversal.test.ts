...
describe('admin attendance reversal migration', () => {
  ...
  it('returns the consumed credit to the exact membership once', () => {
    ...
    expect(rpc).toContain("WHEN v_credit_kind = 'recovery' THEN 'recovery_consumed'")
    expect(rpc).toContain("ELSE 'attendance_consumed'")
    ...
    expect(rpc).toContain('UPDATE public.membership_recovery_credits')
    expect(rpc).toContain('classes_remaining = LEAST(classes_remaining + 1, classes_granted)')
