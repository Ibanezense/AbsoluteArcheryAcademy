...
describe('admin memberships active list and plans catalog', () => {
  ...
  it('supports safe membership edits through the existing admin RPCs', () => {
    ...
    expect(page).toContain('canDeleteExpiredMembership')
    expect(page).toContain('Eliminar vencida')
    expect(page).toContain('Solo se puede eliminar una membresia vencida, historica, cancelada o consumida.')
  ...
  it('keeps the next blocks inside frontend and existing backend protections', () => {
    ...
    expect(page).toContain('Esta accion solo aplica a ciclos vencidos o cerrados y no elimina la membresia activa nueva.')
  })
})
