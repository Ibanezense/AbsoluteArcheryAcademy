...
describe('admin memberships active list and plans catalog', () => {
  ...
  it('supports safe membership edits through the existing admin RPCs', () => {
    ...
    expect(page).toContain("supabase.rpc('admin_get_membership_deletion_preview'")
    expect(page).toContain("supabase.rpc('admin_delete_student_membership'")
    expect(page).not.toContain('canDeleteExpiredMembership')
    expect(page).toContain('isPersistedMembership')
    expect(page).toContain('Eliminar membresia')
    expect(page).not.toContain('Eliminar vencida')
    expect(page).toContain('El servidor verificara que no exista historial de asistencia antes de eliminar.')
    expect(page).toContain('previewData.can_delete')
    expect(page).toContain('previewData.reason')
    expect(page).toContain('buildMembershipDeletionConfirmation')
    expect(page).toContain('formatMembershipDeletionSuccess')
  ...
  it('keeps the next blocks inside frontend and existing backend protections', () => {
    ...
    expect(page).not.toContain('Esta accion solo aplica a ciclos vencidos o cerrados y no elimina la membresia activa nueva.')
  })
})
