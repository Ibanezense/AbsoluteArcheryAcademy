...
-- Fecha: 2025-11-04 (Actualizado 2026-03)
-- Descripción: Calcula métricas clave del dashboard para administradores
--              Se actualizó para leer facturación del mes desde student_membership_payments.
...
BEGIN
  ...
  -- Suma los pagos del mes calendario actual (CORREGIDO PARA MEMBRESÍAS V2)
  ...
  -- 6. Turnos disponibles en la semana
  WITH semana_actual AS (
    SELECT 
      date_trunc('week', (NOW() AT TIME ZONE 'America/Lima')::date)::date AS lunes,
      date_trunc('week', (NOW() AT TIME ZONE 'America/Lima')::date)::date + INTERVAL '6 days' AS domingo
  ...
  -- 7. Ocupación por día de la semana
  WITH semana_actual AS (
    SELECT 
      date_trunc('week', (NOW() AT TIME ZONE 'America/Lima')::date)::date AS lunes,
      date_trunc('week', (NOW() AT TIME ZONE 'America/Lima')::date)::date + INTERVAL '6 days' AS domingo
